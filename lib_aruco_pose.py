...
class ArucoSingleTracker():
    def __init__(self,
        ...
        self._t = geometry_msgs.msg.TransformStamped()
        self._t.header.frame_id = "world"
        self._t.child_frame_id = "aruco"
        self._angle_dist = geometry_msgs.msg.Vector3()
        ...
        while not self._kill:
            ...
            if not ids is None and self.id_to_find in ids[0]:
                ...
                if show_video:
                    ...
                    cv2.putText(frame, str_position, (0, 100), self._font, self._fontscale, (0, 255, 0), 2, cv2.LINE_AA)        
                    
                    #-- Print the marker's attitude respect to camera frame
                    str_attitude = "MARKER Attitude r=%4.0f  p=%4.0f  y=%4.0f"%(math.degrees(roll_marker),math.degrees(pitch_marker),
                                        math.degrees(yaw_marker))
                    cv2.putText(frame, str_attitude, (0, 150), self._font, self._fontscale, (0, 255, 0), 2, cv2.LINE_AA)

                    str_position = "CAMERA Position x=%4.0f  y=%4.0f  z=%4.0f"%(pos_camera[0], pos_camera[1], pos_camera[2])
                    cv2.putText(frame, str_position, (0, 200), self._font, self._fontscale, (0, 255, 0), 2, cv2.LINE_AA)
                    ...
                    cv2.putText(frame, str_attitude, (0, 250), self._font, self._fontscale, (0, 255, 0), 2, cv2.LINE_AA)
                    #a=(tvec[2] * .01)*39.3701#*3.28084
                    #print a
                    str_distance = "CAMERA Distance %.02f in"%((tvec[2] * .01)*39.3701)
                    cv2.putText(frame, str_distance, (0, 300), self._font, self._fontscale, (0, 255, 0), 2, cv2.LINE_AA)
                    #cv2.putText(frame, "%.1f cm -- %.0f deg" % ((tvec[2] * .01), (rvec[2] / math.pi * 180)), (0, 300), self._font, self._fontscale, cv2.LINE_AA)

                    self._t.header.stamp = rospy.Time.now()
                    self._t.transform.translation = tvec
                    self._t.transform.rotation=geometry_msgs.msg.Quaternion(*tf.transformations.quaternion_from_euler(roll_camera,pitch_camera,yaw_camera))
                    quat = quaternion_from_euler (roll_camera, pitch_camera,yaw_camera)
                    print quat
                    self._br.sendTransform(tvec, quat, self._t.header.stamp, self._t.child_frame_id,self._t.header.frame_id)
                    
                    self._angle_dist.x=(tvec[2] * .01)*39.3701
                    self._angle_dist.y=math.degrees(pitch_camera)
                    self._rate.sleep()    
...
if __name__ == "__main__":
    ...
    calib_path  = "/home/rbarnes/catkin_ws/src/ros_tutorials/opencv/src/"
    camera_matrix   = np.loadtxt(calib_path+'cameraMatrix.txt', delimiter=',')
    camera_distortion   = np.loadtxt(calib_path+'cameraDistortion.txt', delimiter=',')                                      
    aruco_tracker = ArucoSingleTracker(id_to_find=72, marker_size=10, show_video=True, camera_matrix=camera_matrix, camera_distortion=camera_distortion)
